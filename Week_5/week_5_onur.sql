...
with Countries AS (SELECT c.Country as "Country",--Sum( od.UnitPrice*od.Quantity*(1-od.Discount)) as 'Sales Amount', 
					       ...
					       FROM Orders o
							...
							HAVING "Category"= "Low" or "Category"= "High"
							Order by "Sales Amount" DESC)
select co.Category as "Sales Category",co.Country as "Country",count(o.OrderID) as "Number of Orders","("|| GROUP_Concat(DISTINCT o.OrderID)||")" as "Order ID", avg(julianday(o.RequiredDate)-julianday(o.ShippedDate)) as "AVG Delay"
From Orders as o
...
-- 								JOIN Customers c On o.CustomerID =c.CustomerID
-- 							Group BY c.Country
-- 							HAVING "Category"= "High" --or "Category"= "High"
-- 							Order by "Sales Amount" DESC)
-- select co.Country as "Country",count(o.OrderID) as "Number of Orders","("|| GROUP_Concat(DISTINCT o.OrderID)||")" as "Order ID", avg(julianday(o.RequiredDate)-julianday(o.ShippedDate)) as "AVG Delay"
-- From Orders as o
-- JOIN Customers as cu on o.CustomerID=cu.CustomerID
-- JOIN Countries as co on co.Country=cu.Country
-- WHERE (julianday(o.RequiredDate)-julianday(o.ShippedDate)) <0
